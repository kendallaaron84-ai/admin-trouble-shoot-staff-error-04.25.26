...
export const Forms: React.FC<FormsProps> = ({ state, currentUserEmail }) => {
  ...
  const [fundSource, setFundSource] = useState<'WFF' | 'AEDC' | ''>('');
  // Get Current Employee Info
  const employee = state.employees.find(e => e.email === currentUserEmail);
  const isAdmin = employee?.role === 'MASTER_ADMIN' || employee?.role === 'ADMIN';
  const isCoach = employee?.functionalRole === 'COACH';
  const showFundingSource = isAdmin || !isCoach;
  const isMarkHillTeam = employee?.managerId === 'e6' || employee?.id === 'e6';
  const isMarketing = employee?.functionalRole === 'MARKETING';

  let autoCategory: BudgetCategory = 'Personnel'; // Default starting point

  // Use the booleans we just defined above
  if (isCoach) {
    autoCategory = 'Contractual'; // Logic: Coaches are billed as Contractual
  } else if (isMarketing) {
    autoCategory = 'Marketing';   // Logic: Marketing hits the AEDC budget cap
  }

  const handleEdit = (log: any) => {
    // 1. Fill the form with the old data
    setSharedDate(log.weekEnding);
    setHours(log.actualHours.toString());
    setJournal(log.journalEntry);
    setFundSource(log.fundingSource);
    
    // 2. Lock in the editing state
    setEditingLogId(log.id);
    setRejectionNote(log.rejectionReason);
    
    // 3. Set the form tab correctly
    setActiveForm('time'); // Ensuring we are on the time tab
    
    // 4. Smooth scroll to top so they see the form is ready
  ...
  const handleTimeSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!employee) {
    console.error("Profile Lookup Failed for:", currentUserEmail);
    alert(`User profile not found for ${currentUserEmail}. Please ensure you have run the Database Seeder or added your profile to Firestore.`);
    return;
  }
    ...
    try {
      ...
      const budgetRate = (employee.plannedProgramBudget && employee.fteOperationalHours) 
        ? (employee.plannedProgramBudget / employee.fteOperationalHours) : 0;
      ...
      const logData = {
        ...
        updatedAt: new Date().toISOString() // Tracking the resubmission time
      };

      if (editingLogId) {
        // UPDATE EXISTING LOG
        const logRef = doc(db, 'timeLogs', editingLogId);
        await updateDoc(logRef, { ...logData, rejectionReason: null }); // Clear the old reason
        setSuccessMsg("Entry updated and resubmitted successfully!");
      } else {
        // CREATE NEW LOG
        await addDoc(collection(db, 'timeLogs'), { ...logData, createdAt: new Date().toISOString() });
    ...
    setErrorMsg(err.message || "Failed to submit time log. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleTravelSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newTravel = {
        employeeId: employee.id,
        fundingSource: fundSource,
        budgetCategory: autoCategory, // Ensure travel is also categorized
        employeeName: `${employee.firstName} ${employee.lastName}`,
        weekEnding: sharedDate,
  ...
  return (
    <div className="space-y-8">
      <div className="max-w-2xl mx-auto bg-white dark:bg-ramp-surface p-8 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800 relative">
        ...
        {/* DATE & FUNDING HEADER */}
        <div className="mb-6 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row md:items-center gap-6">
          <div className="flex-1">
              <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">
                Week Ending / Travel Date
              </label>
              <div className="relative">
                ...
                <input 
                    ...
                    className="w-full pl-10 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded p-2 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-ramp-gold outline-none" 
                />
              </div>
          </div>

          {showFundingSource && (
              <div className="flex flex-col gap-2 pt-4 md:pt-0 border-t md:border-t-0 border-slate-200 dark:border-slate-800">
                ...
                <div className="flex gap-6 items-center">
                    <label className="flex items-center gap-2 cursor-pointer group">
                      <input 
                          ...
                          name="fundSource" 
                          value="WFF" 
                          checked={fundSource === 'WFF'} 
                          onChange={(e) => setFundSource(e.target.value as any)} 
                          className="w-4 h-4 text-ramp-gold border-slate-300 focus:ring-ramp-gold" 
                          required={!isCoach} 
                      />
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-200 group-hover:text-ramp-gold transition-colors">
                          WFF
                      </span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer group">
        ...
        {activeForm === 'time' ? (
          <form onSubmit={handleTimeSubmit} className="space-y-6">
             ...
             <div className="flex-1">
              ...
              <div className="flex items-center h-[38px] px-3 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded shadow-sm">
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-tight ${
                  autoCategory === 'Marketing' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300' :
                  autoCategory === 'Contractual' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300' :
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        {myTimeLogs.length === 0 && myTravels.length === 0 && (
          <p className="text-sm text-gray-400 italic">No history found.</p>
        )}

        {myTravels.map(t => (
                  <div key={t.id} className={`p-4 rounded border-l-4 shadow-sm flex flex-col gap-3 ${
                    t.status === 'Rejected' ? 'bg-red-50 dark:bg-red-900/10 border-red-500' : 'bg-white dark:bg-ramp-surface border-blue-500'
                  }`}>
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="font-bold text-sm dark:text-white flex items-center gap-2 text-slate-900">
                          <Plane size={14}/> Travel: ${t.totalReimbursement.toFixed(2)}
                        </p>
                        <p className="text-xs text-slate-500">{t.weekEnding}</p>
                      </div>
                    ...
                    {t.status === 'Rejected' && (
                      <div className="p-3 bg-white/50 dark:bg-black/20 rounded border border-red-100 dark:border-red-900/50">
                        <div className="flex items-start gap-2">
                          <AlertCircle size={14} className="text-red-600 mt-0.5" />
                          <div className="flex-1">
                            <p className="text-[10px] font-bold text-red-600 uppercase tracking-wider">Reason for Rejection:</p>
                            <p className="text-xs text-slate-700 dark:text-slate-300 italic mb-2">"{t.rejectionReason}"</p>
                            <button 
                              type="button"
                              onClick={() => handleEditTravel(t)}
                              className="text-xs font-bold text-blue-600 dark:text-blue-400 underline hover:text-blue-700 flex items-center gap-1"
                            >
                              Fix & Resubmit Travel
                            </button>
                          </div>
