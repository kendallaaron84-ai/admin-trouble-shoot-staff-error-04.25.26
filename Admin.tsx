...
// --- REJECTION STATES ---
  ...
  const handleReject = async (collectionName: string, id: string) => {
    ...
    try {
      const logRef = doc(db, collectionName, id);
               ...
               <div key={l.id} className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <div className="flex flex-col gap-2 w-full md:w-auto">
                  {rejectingId === l.id ? (
                    ...
                    <div className="flex flex-col gap-2 min-w-[250px] bg-red-50 dark:bg-red-900/10 p-3 rounded border border-red-200">
                      <textarea
                        value={rejectionReason}
                        onChange={(e) => setRejectionReason(e.target.value)}
                        placeholder="Why is this being rejected?"
                        className="text-xs p-2 border rounded dark:bg-black dark:text-white focus:ring-1 focus:ring-red-500 outline-none"
                        rows={2}
                        autoFocus
                      />
                      <div className="flex gap-2">
                        <button 
                          onClick={() => handleReject('timeLogs', l.id)} 
                          className="flex-1 px-2 py-1 bg-red-600 text-white rounded text-[10px] font-bold uppercase hover:bg-red-700"
                        >
                          Confirm Reject
                        </button>
                        <button 
                          onClick={() => { setRejectingId(null); setRejectionReason(''); }} 
                          className="flex-1 px-2 py-1 bg-gray-200 text-gray-700 rounded text-[10px] font-bold uppercase hover:bg-gray-300"
                        >
                          Cancel
                        </button>
                      </div>
                    ...
                    <div className="flex gap-2">
                      <button 
                        onClick={() => handleApprove('timeLogs', l.id, isManager ? 'Pending' : 'Approved')} 
                        className="flex-1 md:flex-none px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded font-bold flex items-center justify-center gap-2 text-xs uppercase"
                      >
                        <Check size={14}/> Approve
                      </button>
                      <button 
                        onClick={() => setRejectingId(l.id)} 
                        className="flex-1 md:flex-none px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded font-bold flex items-center justify-center gap-2 text-xs uppercase"
                      >
                        <X size={14}/> Reject
                      </button>
                    </div>
                ...
                {rejectingId === t.id ? (
                  ...
                  <div className="flex flex-col gap-2 min-w-[250px] bg-red-50 dark:bg-red-900/10 p-3 rounded border border-red-200">
                    <textarea
                      value={rejectionReason}
                      onChange={(e) => setRejectionReason(e.target.value)}
                      placeholder="Reason for travel rejection..."
                      className="text-xs p-2 border rounded dark:bg-black dark:text-white focus:ring-1 focus:ring-red-500 outline-none"
                      rows={2}
                      autoFocus
                    />
                    <div className="flex gap-2">
                      <button 
                        onClick={() => handleReject('travelRequests', t.id)} 
                        className="flex-1 px-2 py-1 bg-red-600 text-white rounded text-[10px] font-bold uppercase hover:bg-red-700"
                      >
                        Confirm Reject
                      </button>
                  ...
                  <div className="flex gap-2">
                    <button 
                      onClick={() => handleApprove('travelRequests', t.id, 'Approved')} 
                      className="flex-1 md:flex-none px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded font-bold flex items-center justify-center gap-2 text-xs uppercase"
                    >
                      <Check size={14}/> Approve
                    </button>
